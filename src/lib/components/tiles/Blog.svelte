				...
				<div class="flex h-full flex-col">
					...
					<span class="mt-3 text-xs text-grey-600 md:text-base font-montserrat">
						{#if item.data.overwrite_publish_date}
							{FormatDate(ph.asDate(item.data.overwrite_publish_date), 'day')}. {FormatDate(
								ph.asDate(item.data.overwrite_publish_date),
								'monthshort'
							)}
							{FormatDate(ph.asDate(item.data.overwrite_publish_date), 'year')}
						{:else}
							{FormatDate(ph.asDate(item.first_publication_date), 'day')}. {FormatDate(
								ph.asDate(item.first_publication_date),
								'monthshort'
							)}
							{FormatDate(ph.asDate(item.first_publication_date), 'year')}
						{/if}
					</span>
				</div>
			</a>
